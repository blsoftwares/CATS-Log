<?php
/*assign data*/
$erating=CatsPillar::model()->findAll();
/*(array('site_id'=>$sitelist['site_id']),array('order'=>'criteria_id ASC'));*/
$rowLabels=array();
$data =array();
foreach($erating as $elist){
	
	 $sql3 = "Select points from site_criteria_history WHERE rating!=0 and site_id='".$sitelist['site_id']."' and criteria_id IN( Select criteria_id from cats_criteria WHERE standard_id IN(select standard_id from cats_standard where element_id IN(Select element_id from cats_element where pillar_id='".$elist['pillar_id']."')))";
...
array_push($rowLabels, $elist['pillar_id']);
...
$chartWidth = 190;
$chartHeight = 80;
$chartXLabel = $sitelist['name_cpa']." - Average Pillar Score";
$chartYLabel = "Score";
$chartYStep = 5;
